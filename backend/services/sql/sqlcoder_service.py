...
def sqlcoder_enabled() -> bool:
    """True when SQLCODER_ENABLED is on (default: auto → on for local non-Vercel)."""
    raw = (os.getenv("SQLCODER_ENABLED") or "auto").strip().lower()
    if raw in ("0", "false", "no", "off"):
        return False
    if raw in ("1", "true", "yes", "on"):
        return True
    # auto: enable locally; disable on serverless/Vercel where GGUF won't fit
...
def get_sqlcoder_llm():
    ...
    with _lock:
        ...
        try:
            ...
            logger.info(
                "Loading SQLCoder once from %s (n_ctx=%s n_gpu_layers=%s n_threads=%s)",
                model_path,
                _n_ctx(),
                _n_gpu_layers(),
                _n_threads(),
            ...
            _load_error = f"Failed to load SQLCoder: {exc}"
...
def build_sqlcoder_prompt(
    ...
    *,
    ...
    extra = format_requirements_for_sqlcoder(requirement_contract)
    if extra:
        user_q = f"{user_q}{extra}"
    return _SQLCODER_PROMPT.format(
